...
**Release summary**: This is a hardening, automation-debt, and documentation cycle — **no new end-user product features shipped**. It closes the v0.10.6 code-quality reviewer backlog (Groups A + D), adds a blocking pre-push gate (Layer 2), fills the in-repo wiki tree (~47 pages) with generators wired into CI, and runs a doc-wide CLI-example accuracy sweep that surfaced two multi-cycle naming/usage drifts (see §12 corrections). **3790 tests pass / 14 skipped across 281 source files (was 279 v0.10.6); mypy strict 0/0; ruff clean.** (The Phase-E version-tracker + README + commit-subject tooling added the tests beyond the mid-cycle 3666 baseline.) Workspace ships **8 PyPI packages** unchanged from v0.10.6 (no new packages this cycle, no LL-V105-1 partial-publish recurrence risk). Per-cycle detail in [`docs/v0.10.7-plan.md`](docs/v0.10.7-plan.md).
...
- **Never-skip version-tracking.** New declarative manifest `scripts/version_tracked_files.yaml` (`tracked` + `frozen` lists) is the single source of truth that both the bumper and the new gate read. `scripts/bump_version.py` is refactored to drive its replacements from the manifest — and now also tracks `CITATION.cff` (`version` + `date-released`) and the `README.md` container-image tag (tightly anchored), which it previously skipped silently. New `scripts/check_version_consistency.py` hard-fails (pre-tag, wired as the 8th `.githooks/pre-push` check) on any project-version literal in a file that is neither `tracked` nor `frozen`, so a newly-added version reference can no longer drift unnoticed. The marketplace plugin is reclassified `frozen` (versions on its own cadence, no longer auto-bumped). See commits `eedde81`, `5f44982`, `458a94c`.
