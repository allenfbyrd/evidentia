"""Tests for the v0.2.1 ``controlbridge.yaml`` config loader."""
...
import pytest
from controlbridge.config import (
    ControlBridgeConfig,
    _expand_env_vars,
    find_config_file,
    get_default,
...
def _write_yaml(path: Path, content: str) -> Path:
    ...
    from controlbridge.config import _load_config_cached
...
def test_find_config_finds_file_in_cwd(tmp_path: Path) -> None:
    cfg = tmp_path / "controlbridge.yaml"
...
def test_find_config_walks_to_parent(tmp_path: Path) -> None:
    ...
    child.mkdir()
    (tmp_path / "controlbridge.yaml").write_text("organization: Parent")
    found = find_config_file(start=child)
    assert found == (tmp_path / "controlbridge.yaml")
...
def test_find_config_returns_none_when_missing(tmp_path: Path) -> None:
    ...
    # higher ancestor (like the project's own controlbridge.yaml).
    deep = tmp_path / "a" / "b" / "c"
    ...
    assert result is None or tmp_path in result.parents or result == tmp_path / "controlbridge.yaml"
...
def test_load_config_none_when_no_file() -> None:
    ...
    assert isinstance(cfg, ControlBridgeConfig)


def test_load_config_happy_path(tmp_path: Path) -> None:
    path = _write_yaml(
        tmp_path / "controlbridge.yaml",
...
def test_load_config_empty_yaml_yields_defaults(tmp_path: Path) -> None:
    ...
    path = _write_yaml(tmp_path / "controlbridge.yaml", "")
    cfg = load_config(path)
...
def test_load_config_warns_on_large_framework_set(tmp_path: Path) -> None:
    ...
    path = _write_yaml(tmp_path / "controlbridge.yaml", f"frameworks:\n{many}\n")
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        load_config(path)
...
def test_load_config_malformed_yaml_raises(tmp_path: Path) -> None:
    ...
    path = _write_yaml(tmp_path / "controlbridge.yaml", "- just a list\n- of items\n")
...
def test_expand_env_vars_in_config(tmp_path: Path, monkeypatch) -> None:
    ...
    path = _write_yaml(
        tmp_path / "controlbridge.yaml", "organization: ${SPECIAL_ORG}\n"
    )
    cfg = load_config(path)
...
def test_get_default_cli_wins_over_env_and_yaml(monkeypatch) -> None:
    cfg = ControlBridgeConfig(organization="YamlOrg")
...
def test_get_default_falls_through_to_builtin_default(monkeypatch) -> None:
    cfg = ControlBridgeConfig()  # no org set
    monkeypatch.delenv("CB_TEST_ORG", raising=False)
    assert (
        get_default(
