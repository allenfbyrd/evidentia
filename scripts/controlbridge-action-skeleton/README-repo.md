# controlbridge-action

Reusable GitHub Action wrapping the [ControlBridge](https://github.com/allenfbyrd/controlbridge) CLI for PR-level **compliance-as-code**. Drop-in replacement for the [80-line workflow template](https://github.com/allenfbyrd/controlbridge/blob/main/docs/github-action/workflow-example.yml):
...
1. Runs `controlbridge gap analyze` on the PR branch's inventory
...
| `comment-tag` | no | `controlbridge-gap-diff` | Marker for de-duplicating PR comments |
| `github-token` | no | `${{ github.token }}` | Token used to post PR comments |
| `controlbridge-version` | no | `latest` | Pin to a specific ControlBridge version |
...
Drop into `.github/workflows/controlbridge.yml`:
