"""Pure-functional mapping between ControlBridge gaps and Jira issues.

Extracted from :mod:`controlbridge_integrations.jira.client` so these
...
from controlbridge_core.models.gap import ControlGap, GapSeverity, GapStatus
...
def gap_to_create_request(gap: ControlGap) -> dict[str, object]:
    ...
    Produces a structured body with:
      ...
      - ``labels``: the framework id + ``controlbridge`` + severity
    ...
    lines.extend(
        [
            "",
            f"Tracked by ControlBridge gap id: {gap.id}",
        ]
    )
...
def _enum_value(value: object) -> str:
    ...
    ControlBridge's Pydantic models use ``use_enum_values=True`` which
