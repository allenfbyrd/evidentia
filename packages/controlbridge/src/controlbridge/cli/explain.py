"""`controlbridge explain` — plain-English control explanations via LLM.
...
Usage::
    ...
    controlbridge explain CC6.1 --framework soc2-tsc --model claude-opus-4
    controlbridge explain AC-2 --framework nist-800-53-rev5 --refresh
"""
...
def explain_control(
    ...
    framework: str | None = typer.Option(
        ...
        help=(
            ...
            "Reads from `controlbridge.yaml` when --framework is omitted and "
    ...
    model: str | None = typer.Option(
        ...
        "-m",
        help=(
            "LLM model. Defaults to $CONTROLBRIDGE_LLM_MODEL, then "
            "controlbridge.yaml llm.model, then 'gpt-4o'."
        ),
    ),
    ...
    cfg: ControlBridgeConfig = (
        cfg_obj if cfg_obj is not None else ControlBridgeConfig()
    )
    ...
    if not resolved_fw:
        ...
        if len(fws) == 1:
            ...
            console.print(
                f"[red]Multiple frameworks in controlbridge.yaml ({len(fws)}); "
    ...
    if not resolved_model:
        from controlbridge_ai.client import get_default_model
    ...
    if control is None:
        console.print(
            ...
            f"'{resolved_fw}'.[/red]\n"
            f"[dim]Run `controlbridge catalog show {resolved_fw}` to see "
...
# ---------------------------------------------------------------------------
# Cache management commands (scoped under `controlbridge explain cache ...`)
# ---------------------------------------------------------------------------
...
def clear(
    ...
    from controlbridge_ai.explain.cache import clear_cache, get_cache_dir
...
def where() -> None:
    ...
    from controlbridge_ai.explain.cache import get_cache_dir

    path = get_cache_dir()
    console.print(str(path))
    console.print("[dim]Override via $CONTROLBRIDGE_EXPLAIN_CACHE_DIR[/dim]")
