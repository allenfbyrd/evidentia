...
into :mod:`controlbridge_core.init_wizard` so the GUI's onboarding wizard
...
def init(
    ...
    organization: str = typer.Option(
        ...
        "--organization",
        "-o",
        help="Organization name used in the generated controlbridge.yaml.",
    ),
    ...
    # v0.4.0: delegate to controlbridge_core.init_wizard \u2014 same code path
    # the GUI's /api/init/wizard endpoint uses.
    try:
        files = {
            "controlbridge.yaml": generate_controlbridge_yaml(
    ...
    console.print(
        "  2. Run: [cyan]controlbridge gap analyze --inventory my-controls.yaml "
    ...
    console.print(
        "  4. Launch the web UI: [cyan]controlbridge serve[/cyan] "
        r"(requires the \[gui] extra: [cyan]pip install 'controlbridge[gui]'[/cyan])"
    )
