"""`controlbridge catalog` — framework catalog exploration and user-import.
...
def import_catalog(
    ...
    catalog_dir: Path | None = typer.Option(
        None,
        "--catalog-dir",
        help="Override user catalog directory (also via CONTROLBRIDGE_CATALOG_DIR).",
    ),
) -> None:
    ...
    \b
    1. Direct JSON:      controlbridge catalog import ./my-iso27001.json
    2. OSCAL profile:    controlbridge catalog import --profile profile.json --catalog source.json
    3. Replace stub:     controlbridge catalog import --framework-id soc2-tsc ./my-tsc.json --force
    ...
    if profile is not None:
        ...
        # Write resolved catalog as a standard ControlBridge JSON
