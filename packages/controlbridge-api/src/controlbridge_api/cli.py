"""`controlbridge serve` implementation.

Invoked from :mod:`controlbridge.cli.main` via the `serve` Typer command.
...
(``npm run dev`` in ``packages/controlbridge-ui/``) on port 5173; the
...
def serve(
    *,
    ...
    """Spawn uvicorn serving the ControlBridge API + web UI.
    ...
    try:
        ...
        raise ImportError(
            ...
            "`pip install 'controlbridge[gui]'` or "
    ...
    if host not in ("127.0.0.1", "localhost", "::1"):
        logger.warning(
            ...
            "ControlBridge has no auth in v0.4.0 — anyone who can reach this "
    ...
    # controlbridge_api.app.create_app via module-level env vars so they
    # survive the subprocess boundary.
    env = os.environ.copy()
    if offline:
        env["CONTROLBRIDGE_API_OFFLINE"] = "1"
    if dev:
        env["CONTROLBRIDGE_API_DEV"] = "1"

    cmd = [
        sys.executable,
        "-m",
        "uvicorn",
        "controlbridge_api.app:app",
        "--host",
        host,
        "--port",
        str(port),
...
def resolve_static_dir() -> Path:
    ...
    from controlbridge_api.app import STATIC_DIR
