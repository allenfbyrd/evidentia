...
Mirrors the ``controlbridge doctor`` CLI output for browser consumption,
...
from controlbridge_api.schemas import AirGapCheck, AirGapCheckResponse
...
async def doctor() -> dict[str, object]:
    """Return a diagnostic summary of the ControlBridge installation.
...
async def check_air_gap() -> AirGapCheckResponse:
    ...
    checks.append(
        AirGapCheck(
            subsystem="web_ui",
            status="ok",
            detail="`controlbridge serve` binds to 127.0.0.1 by default",
        )
    )
