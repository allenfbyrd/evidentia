...
from controlbridge_api.schemas import LlmProviderState, LlmStatusResponse
...
async def llm_status() -> LlmStatusResponse:
    ...
    from controlbridge_core.network_guard import LOCAL_LLM_PREFIXES

    model = get_default_model()
    local_configured = any(model.lower().startswith(p) for p in LOCAL_LLM_PREFIXES) or bool(
        os.environ.get("CONTROLBRIDGE_LLM_API_BASE")
    )
    providers["ollama"] = LlmProviderState(
        ...
        source=(
            ...
            else ("env:CONTROLBRIDGE_LLM_API_BASE" if local_configured else None)
        ),
    )
