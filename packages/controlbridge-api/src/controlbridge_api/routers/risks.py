...
reuses :class:`controlbridge_ai.RiskStatementGenerator` (which already
...
from controlbridge_core.models.gap import ControlGap, GapAnalysisReport
from fastapi import APIRouter, HTTPException
from sse_starlette.sse import EventSourceResponse

from controlbridge_api.schemas import RiskGenerateRequest
...
async def _stream_risk_generation(
    ...
    try:
        from controlbridge_ai.risk_statements.generator import (
            RiskStatementGenerator,
        )
    except ImportError as e:  # pragma: no cover — controlbridge-ai is required
        yield json.dumps({"phase": "error", "detail": f"controlbridge-ai not available: {e}"})
        return
