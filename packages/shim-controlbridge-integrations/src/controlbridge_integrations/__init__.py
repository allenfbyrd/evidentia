"""DEPRECATED — controlbridge-integrations has been renamed to evidentia-integrations.

This transitional shim re-exports everything from `evidentia_integrations` and aliases
every submodule into `controlbridge_integrations.*` via `sys.modules`, so pre-rename imports
like `from controlbridge_integrations.submodule import Foo` continue to work.
...
Then rewrite `controlbridge_integrations` to `evidentia_integrations` in your imports.
"""
...
_OLD_PYPI = "controlbridge-integrations"
_NEW_PYPI = "evidentia-integrations"
_OLD_MODULE = "controlbridge_integrations"
_NEW_MODULE = "evidentia_integrations"
