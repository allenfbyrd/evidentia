...
Requires the ``[aws]`` extra (``pip install 'controlbridge-collectors[aws]'``).
...
:mod:`controlbridge_collectors.aws.mapping`. Unmapped findings get an
...
from controlbridge_core.models.finding import FindingStatus, SecurityFinding
...
class AwsCollector:
    ...
    standard boto3 chain; no ControlBridge-specific auth knob.
...
def _severity_from_label(label: str) -> Severity:
    """Map Security Hub severity label -> ControlBridge Severity.
