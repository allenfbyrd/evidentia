"""AWS evidence collector for ControlBridge.

Connects to AWS Config + Security Hub + (in v0.5.1) IAM Access Analyzer,
pulls findings, and returns them as typed
:class:`controlbridge_core.models.finding.SecurityFinding` instances
pre-mapped to NIST 800-53 control IDs.

Install: ``pip install 'controlbridge-collectors[aws]'`` (pulls in
...
from controlbridge_collectors.aws.collector import (
    AwsCollector,
    AwsCollectorError,
)
