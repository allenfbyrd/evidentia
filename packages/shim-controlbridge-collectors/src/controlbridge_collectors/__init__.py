"""DEPRECATED — controlbridge-collectors has been renamed to evidentia-collectors.

This transitional shim re-exports everything from `evidentia_collectors` and aliases
every submodule into `controlbridge_collectors.*` via `sys.modules`, so pre-rename imports
like `from controlbridge_collectors.submodule import Foo` continue to work.
...
Then rewrite `controlbridge_collectors` to `evidentia_collectors` in your imports.
"""
...
_OLD_PYPI = "controlbridge-collectors"
_NEW_PYPI = "evidentia-collectors"
_OLD_MODULE = "controlbridge_collectors"
_NEW_MODULE = "evidentia_collectors"
