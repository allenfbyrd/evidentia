...
1. :mod:`controlbridge_ai.client` — every LLM completion call checks the
...
2. :mod:`controlbridge_core.catalogs.loader` — ``catalog import --from-url``
   refuses non-loopback URLs.
3. :mod:`controlbridge.cli.doctor` — ``controlbridge doctor --check-air-gap``
...
Design note: a module-level flag rather than contextvars. ControlBridge's
...
def set_offline(enabled: bool) -> None:
    ...
    to ``controlbridge serve``). Explicit enable/disable avoids the subtle
...
class OfflineViolationError(Exception):
    ...
    ----------
    subsystem
        Which ControlBridge module flagged the violation
