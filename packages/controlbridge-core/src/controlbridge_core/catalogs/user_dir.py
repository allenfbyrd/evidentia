...
- Windows:  ``%APPDATA%\\ControlBridge\\catalogs\\``
- macOS:    ``~/Library/Application Support/controlbridge/catalogs/``
- Linux:    ``~/.local/share/controlbridge/catalogs/``

Override with the ``CONTROLBRIDGE_CATALOG_DIR`` environment variable
...
AICPA TSC JSON and have ``controlbridge catalog show soc2-tsc CC6.1``
...
CATALOG_DIR_ENV_VAR = "CONTROLBRIDGE_CATALOG_DIR"
...
def get_user_catalog_dir(override: Path | None = None) -> Path:
    ...
    2. ``CONTROLBRIDGE_CATALOG_DIR`` environment variable
    ...
    # "controlbridge" = app name, "ControlBridge" = app author
    return Path(user_data_dir("controlbridge", "ControlBridge")) / "catalogs"
...
def resolve_catalog_path(
    ...
    if bundled_data_dir is None:
        from controlbridge_core.catalogs.loader import DATA_DIR
