"""Starter-file generators shared by ``controlbridge init`` and the web UI.

v0.4.0 extracts the starter-YAML templates and framework-recommendation
rules from ``controlbridge.cli.init`` into this module so:

- The CLI's Typer-prompt flow (``controlbridge init``) continues to work
...
_CONTROLBRIDGE_YAML_TEMPLATE = """\
# ControlBridge project configuration.
#
# v0.2.1: this file is read by every `controlbridge` command (was
...
def generate_controlbridge_yaml(
    organization: str,
    frameworks: list[str],
    *,
    ...
    """Render a ``controlbridge.yaml`` starter file.
    ...
    return _CONTROLBRIDGE_YAML_TEMPLATE.format(
        organization=organization,
...
# you listed in controlbridge.yaml (NIST 800-53, SOC 2, HIPAA, etc.).
#
...
""",
    "empty": "  # Add your controls here. Run `controlbridge catalog show <framework>` to see available IDs.\n",
}
