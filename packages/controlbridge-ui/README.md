...
React + Vite + TypeScript + shadcn/ui frontend for **ControlBridge**. Served by the `controlbridge-api` FastAPI backend.

> **Note:** This is a Node/Vite project colocated in the ControlBridge monorepo. It is **not** a Python workspace member — `uv` does not touch this directory. End users never build the frontend themselves; built assets are bundled inside the `controlbridge-api` wheel at release time.
...
- **Playwright** — E2E smoke tests against `controlbridge serve`
...
controlbridge serve --dev   # wires permissive CORS for :5173
```
...
Produces `dist/` — static HTML + hashed JS/CSS assets. At release time the GitHub Actions workflow copies `dist/*` into `packages/controlbridge-api/src/controlbridge_api/static/` so the Python wheel ships self-contained.
...
npm run e2e               # Playwright E2E (requires `controlbridge serve` in another terminal)
```
