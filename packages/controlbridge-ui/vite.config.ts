...
/**
 * Vite config for the ControlBridge web UI.
 *
 * Build output goes to dist/ by default. At release time, the GitHub
 * Actions release workflow copies dist/* into packages/controlbridge-api/
 * src/controlbridge_api/static/ so the FastAPI server can serve the SPA
 * from inside its wheel.
 *
 * Dev mode: Vite serves at :5173; the backend FastAPI (started via
 * `controlbridge serve --dev`) runs at :8000 with permissive CORS so
