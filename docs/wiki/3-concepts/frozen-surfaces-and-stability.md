...
- **Pydantic model fields** (`evidentia_core.models.*`). All exported model classes have stable field names and types. Adding an optional field (with a default) is a minor change; renaming, removing, or retyping a field is a major trigger. The frozen set spans 48+ classes across 18 modules — `SecurityFinding` / `Finding` / `ComplianceStatus` / `FindingStatus`, `ControlGap` / `Milestone` / `POAMState`, `CrosswalkMapping`, `CatalogControl` / `ControlCatalog`, the AI-governance and RBAC and retention models, and more (see the [data model](data-model.md) page and the api-stability table for the full roster). A serialization guarantee rides along: JSON output of a frozen model at version N must deserialize under version N+1 within the same major (field *ordering* is not guaranteed).
